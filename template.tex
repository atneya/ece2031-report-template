...
% Some font installation may be required. MikTex or texlive-extra fonts is recommended. 
...
% Figures should not float per formatting specification, so we use the H specifier for all figures.
\begin{figure}[H]
   \centering
...
\begin{figure}[H]
   \centering % Necessary since centered figures are required.
   \includegraphics{complicated_circuit.png}
      \caption{Circuit to light LED G8 when robot motors are enabled, and flash it at 10Hz when robot battery voltage is below minimum threshold.} % Longer two line caption. 
\end{figure}   
...
\begin{figure}[H] % An explicitly inserted code fragment. See appendix for file import. Whitespace should be correctly pasted, as LaTeX follows whitespace verbatim.
	\centering % default listing language is VHDL for highlighting
	\begin{lstlisting} 
	-- ORGATE.VHD (VHDL)
...
\begin{table}[H] % Explicitly inserted table, with correct caption (title) format from setup.
